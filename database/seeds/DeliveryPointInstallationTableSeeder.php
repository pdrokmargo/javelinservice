...
class DeliveryPointInstallationTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $main_env = 'mainqa';
        try{
        $delivery_point = DB::connection($main_env)->table('delivery_points')->where('token', $token)->first();
        ...
        $last_sync->save();
        $contracts = DB::connection($main_env)->table('delivery_contracts')->get();
        ...
        foreach($syncs->value as $sync){
            ...
            if($sync['down'] == true && ($table != 'products' && $table != 'active_ingredients' && $table != 'active_ingredients_pharmaceutical_drugs' && $table != 'pharmaceutical_drugs' && $table != 'pharmaceutical_drugs_products')){
                ...
                if($table == 'companies'){
                    DB::connection($connection)->table($table)->insert(json_decode(DB::connection($main_env)->table($table)->where('id', $delivery_point->company_id)->get(), true));
                }
                // Affiliates installation: DONE
                else if($table == 'affiliates'){ 
                    foreach($a_contracts as $contract){
                        DB::connection($connection)->table($table)->insert(json_decode(DB::connection($main_env)->table($table)->where('delivery_contract_id', $contract['id'])->get(), true));
                    }
                }
                // Products installation: DONE
                else if($table == 'products'){ 
                    $collection = DB::connection($main_env)->table($table)->get();
                ...
                // Contracts installation: DONE
                    ...
                    // $contracts = DB::connection($main_env)->table($table)->get();
                ...
                // Users installation: DONE
                    ...
                    // DB::connection($connection)->table('privileges')->insert(json_decode(DB::connection($main_env)->table('privileges')->where('user_profile_id', 1)->get(), true));
                    // DB::connection($connection)->table('users_privileges')->insert(json_decode(DB::connection($main_env)->table('users_privileges')->where('user_profile_id', 1)->where('user_id', '7ddc0553-0b27-49f1-a1b7-445777a72d0e')->where('company_id', $delivery_point->company_id)->get(), true));
                    $users = [];
                    foreach(json_decode($delivery_point->users, true) as $u){
                        $users[] = $u["user_id"];
                    }
                    DB::connection($connection)->table($table)->insert(json_decode(DB::connection($main_env)->table($table)->whereIn('id', $users)->get(), true));
                }
                else {
                    // General installation
                    DB::connection($connection)->table($table)->insert(json_decode(DB::connection($main_env)->table($table)->get(), true));
                }
            }
            dump($table.' finalizado');
        }
        DB::connection($main_env)->table('delivery_points')->where('token', $token)->update(['installed' => true]);
        }catch(Exception $e){
            echo $table.'\n\n';
            echo $e;
        }
