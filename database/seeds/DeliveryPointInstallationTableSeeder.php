    ...
    public function run()
    {
        ...
        foreach(json_decode($syncs->value, true) as $sync){
            ...
            if($sync['down'] == true && ($table != 'products' && $table != 'active_ingredients' && $table != 'active_ingredients_pharmaceutical_drugs')){
                DB::connection('local')->table($table)->delete();

                // Companies installation: DONE
                if($table == 'companies'){
                    DB::connection('local')->table($table)->insert(json_decode(DB::connection('main')->table($table)->where('id', $delivery_point->company_id)->get(), true));
                }
                // Affiliates installation: DONE
                else if($table == 'affiliates'){ 
                    foreach($a_contracts as $contract){
                        DB::connection('local')->table($table)->insert(json_decode(DB::connection('main')->table($table)->where('delivery_contract_id', $contract['id'])->get(), true));
                    }
                }
                // Products installation: DONE
                    ...
                    foreach($collection as $c){
                        DB::connection('local')->table($table)->insert((array)$c);
                    }
                }
                ...
                // Contracts installation: DONE
                    ...
                    dump($a_contracts);
                    DB::connection('local')->table($table)->insert($a_contracts);
                ...
                // Users installation: DONE
                    ...
                    DB::connection('local')->table($table)->insert([
                        [
                    ...
                    DB::connection('local')->table('privileges')->insert(json_decode(DB::connection('main')->table('privileges')->where('user_profile_id', 1)->get(), true));
                    DB::connection('local')->table('users_privileges')->insert(json_decode(DB::connection('main')->table('users_privileges')->where('user_profile_id', 1)->where('user_id', '7ddc0553-0b27-49f1-a1b7-445777a72d0e')->where('company_id', $delivery_point->company_id)->get(), true));
                    $users = [];
                    foreach(json_decode($delivery_point->users, true) as $u){
                        $users[] = $u["user_id"];
                    }
                    DB::connection('local')->table($table)->insert(json_decode(DB::connection('main')->table($table)->whereIn('id', $users)->get(), true));
                }
                else {
                    // General installation
                    DB::connection('local')->table($table)->insert(json_decode(DB::connection('main')->table($table)->get(), true));
                }
            }
        }
